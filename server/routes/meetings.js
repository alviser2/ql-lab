...
function meetingVisibleToUser(userId, userRole, m, db) {
  ...
  if (userRole === 'r-vice-director') {
    ...
    const managed = db.prepare('SELECT 1 FROM vice_director_departments WHERE vice_director_id = ? AND department_id = ?').get(userId, m.department_id)
    if (managed) return true
    if (m.created_by_id === userId) return true
    if (m.secretary_id === userId) return true
    if (m.chairperson_id === userId) return true
    const attendee = db.prepare('SELECT 1 FROM meeting_attendees WHERE meeting_id = ? AND user_id = ?').get(m.id, userId)
    return !!attendee
  }
  if (userRole === 'r-dept-head') {
    const me = db.prepare('SELECT dept_id FROM users WHERE id = ?').get(userId)
    if (m.department_id === me?.dept_id) return true
    if (m.chairperson_id === userId) return true
    if (m.secretary_id === userId) return true
    const attendee = db.prepare('SELECT 1 FROM meeting_attendees WHERE meeting_id = ? AND user_id = ?').get(m.id, userId)
    return !!attendee
...
function enrichMeeting(m, db) {
  const attendees = db.prepare('SELECT u.id, u.full_name FROM meeting_attendees ma JOIN users u ON u.id = ma.user_id WHERE ma.meeting_id = ?').all(m.id)
  const minutes = db.prepare('SELECT * FROM meeting_minutes WHERE meeting_id = ?').get(m.id)
  return { ...m, attendeeIds: attendees.map(a => a.id), attendees, minutes: minutes || {} }
}

function meetingAccessibleToUser(userId, userRole, m, db) {
  // For secretary/chair checks
  return {
    isSecretary: m.secretary_id === userId,
    isChair: m.chairperson_id === userId,
    isDirector: userRole === 'r-director',
    isViceDirector: userRole === 'r-vice-director',
...
router.get('/:id', authenticate, (req, res) => {
  ...
  if (!meetingVisibleToUser(userId, userRole, m, db)) {
    return res.status(403).json({ error: 'Không có quyền xem cuộc họp này' })
  }
...
router.post('/', authenticate, (req, res) => {
  ...
  if (userRole !== 'r-director' && userRole !== 'r-vice-director') {
    return res.status(403).json({ error: 'FORBIDDEN_SCHEDULE' })
  }
  ...
  if (!title || !meeting_date) {
    return res.status(400).json({ error: 'Thiếu tiêu đề hoặc ngày họp' })
  }
  if (!secretary_id) {
    return res.status(400).json({ error: 'NO_SECRETARY' })
  }
  ...
  `).run(
    id, title,
    document_number || null, document_place || null,
    document_day || null, document_month || null, document_year || null,
    meeting_date, start_time || null, end_time || null, location || null,
    chairperson_id || userId, secretary_id, department_id || null,
    userId,
  )
...
router.patch('/:id', authenticate, (req, res) => {
  ...
  if (!m) return res.status(404).json({ error: 'Không tìm thấy cuộc họp' })
  if (m.status === 'approved') return res.status(400).json({ error: 'Biên bản đã duyệt, không sửa được' })

  // Permission check: secretary or director/vice-director
  const canEdit =
    m.secretary_id === userId ||
    m.chairperson_id === userId ||
    userRole === 'r-director' ||
    userRole === 'r-vice-director'

  if (!canEdit) return res.status(403).json({ error: 'FORBIDDEN_EDIT' })

  const {
    title, document_number, document_place, document_day, document_month, document_year,
    meeting_date, start_time, end_time, location,
    chairperson_id, secretary_id, department_id,
    attendee_ids,
    content_raw, conclusion, minutes,
  } = req.body
  ...
  if (minutes && typeof minutes === 'object') {
    ...
    if (minuteUpdates.length > 0) {
      ...
      db.prepare(`UPDATE meeting_minutes SET ${minuteUpdates.join(', ')} WHERE meeting_id = ?`).run(...minuteValues)
    }
  }
...
router.post('/:id/approve', authenticate, (req, res) => {
  ...
  if (userRole !== 'r-director') return res.status(403).json({ error: 'Chỉ Giám đốc mới duyệt được' })
  if (m.status === 'approved') return res.status(400).json({ error: 'Đã duyệt rồi' })
